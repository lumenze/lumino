...
const PERSONAS = [
  ...
  { key: 'customer', label: 'Customer', name: 'Priya Nair', initials: 'PN', color: '#2563EB', desc: 'Sees guides & notifications' },
] as const;
...
export default function PersonaSwitcher() {
  ...
  function switchRole(newRole: string) {
    ...
    setRole(newRole);
    setOpen(false);
    if ((window as { Lumino?: { destroy?: () => void } }).Lumino?.destroy) {
      (window as { Lumino?: { destroy?: () => void } }).Lumino?.destroy?.();
    }
  ...
  return (
    <div style={{
      ...
      {open && (
        <div style={{
          position: 'absolute', bottom: 52, left: 0, width: 250,
          background: '#0F172A', borderRadius: 12, padding: 8,
          ...
          {PERSONAS.map((p) => (
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.45)' }}>{p.desc}</div>
              </div>
            </button>
          ))}
        </div>
