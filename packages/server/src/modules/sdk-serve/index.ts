...
export async function registerSdkServeModule(app: FastifyInstance): Promise<void> {
  ...
  app.get('/sdk/v1/lumino.js', async (_request, reply) => {
    try {
      // In production, serve from built dist
      const sdkPath = resolve(process.cwd(), '../sdk/dist/lumino.js');
      const content = await readFile(sdkPath, 'utf-8');
      reply
  ...
  app.get('/sdk/v1/lumino.js.map', async (_request, reply) => {
    try {
      const mapPath = resolve(process.cwd(), '../sdk/dist/lumino.js.map');
  ...
  var Lumino = {
    ...
    _showDevNotification: function(wt) {
      ...
      document.getElementById('lm-dev-start').onclick = function() {
        ...
        console.log('[Lumino Dev Shim] Steps:', def.steps.length);
        console.log('[Lumino Dev Shim] Build the SDK (pnpm build:sdk) for full player experience.');
      };
    },
